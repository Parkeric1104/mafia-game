...
- **낮(아침)** → "☀️ 아침이 되었습니다" 안내 후 사망자 공개·토론
- **투표** → 다수결로 처형 (동률이면 부결)
- **투표 결과** → 처형 결과를 보여준 뒤 다시 밤으로
- **승리** — 시민: 마피아 전멸 / 마피아: 마피아 수 ≥ 시민 수
- **호스트리스**: 타이머가 끝나거나 전원이 행동을 마치면, 접속한 누군가의 브라우저가 Firebase 트랜잭션으로 페이즈를 자동 전진시킵니다. 방장이 나가도 게임은 계속됩니다.

## 권한 (관리자 / 참여자)

- **관리자**: 홈에서 "방 만들기"를 누르고 **관리자 코드**를 입력해야 방을 개설할 수 있습니다. (코드는 `VITE_ADMIN_CODE`, 기본값 `mafia2026`)
- **참여자**: "게임 참여"에서 **현재 열린 방 목록**을 보고 골라서 입장합니다. (게임이 시작된 방은 목록에서 숨겨짐)

> 참고: 관리자 코드는 클라이언트 번들에 포함되는 가벼운 UX 게이트입니다(실제 인증 아님). 외부에 공개 운영하려면 서버/인증 기반 검증을 추가하세요.
